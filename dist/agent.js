...
export function parseJsonResponse(raw) {
    ...
    if (!result.success) {
        throw new ModelResponseError(`Model returned an invalid action payload: ${formatZodError(result.error)}. Raw response: ${raw}`, raw);
    }
    return result.data;
}
...
function createTrace(goal, options) {
    return {
        goal,
...
async function writeTrace(trace, traceDirectory) {
...
export async function runAgent(userGoal, dependencies = {}, options = {}) {
    ...
    try {
        for (let step = 1; step <= resolvedOptions.maxSteps; step += 1) {
            ...
            if (action.type === "plan") {
                ...
                messages.push({
                    role: "user",
                    content: "Plan received. Continue by choosing the next required tool_call, or return final if no tools are needed."
                });
                continue;
            }
            if (action.type === "final") {
